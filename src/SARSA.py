...
class SARSA:
    ...
    def choose_greedy(self, state: Tuple[int, ...]) -> int:
        action_probabilities = np.array(self.Q(tf.convert_to_tensor([state])))
        return np.argmax(action_probabilities)

    def choose_softmax(self, state: Tuple[int, ...], temperature: int) -> int:
        action_probabilities = np.array(self.Q(tf.convert_to_tensor([state])))        action_probabilities = action_probabilities * np.array(valid_actions)
    ...
    def fit(self) -> None:
        # Same as in NN-criitc?
        # history = self.Q.fit(X, Y)

        # Used for visualization
        # self.loss_history.append(history.history["loss"][0])  # type: ignore
        self.epsilon_history.append(self.epsilon)

        self.epsilon *= self.epsilon_decay_rate  # decay epsilon
